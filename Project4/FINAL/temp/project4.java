 ...
 *
 *Pass 2 is created using lstFile class and pass2 class. It figures out the PC relative addressing and direct addressing. Checks for base if
 *if the PC relative address is out of range and then uses the base to figure out the base addressing. 
 * 
 */
...
public class project4 {
   ...
   storeStuff(table,table1,table2,args[0]);
   	
   }
   
   public static void storeStuff(hashTable table1,hashTable table3,hashTable table4, String args) throws Exception
   {
	   
	   
	   
	   java.io.File table = new java.io.File(args);
	   ...
	   while(input2.hasNextLine())
	   {
		   ...
		   {	
			   ...
			   {
				   //store the address
				   address.add(reader.get(i).substring(18,22).trim());
				   a++;
				   operands.add(reader.get(i).substring(18,22).trim());
				   o++;
			   ...
			   else
			   {
				   ...
				   {	
					   if(reader.get(i).substring(18,28).trim().contains(",") && table1.getFormat(reader.get(i).substring(9,16).trim()) == 2)
					   {
						   String check1 = reader.get(i).substring(9,16).trim();
						   if(table1.getFormat(check1) == 2)
						   {
							   operands.add(reader.get(i).substring(18,29).trim());
							   o++;
						   }
					   }
					   
					   else if(reader.get(i).substring(18, 28).trim().contains(","))
				   		{
				   ...
				   {	
					   ...
					   else if(reader.get(i).substring(18).trim().contains(","))
					   {
				   ...
				   {
					   mneumoics.add(reader.get(i).substring(9,16).trim());
					   
				   }
				   else
				   {
					   mneumoics.add(reader.get(i).substring(9).trim());
				   }
	   ...
	   //display the table
	 //table3.displayTable();
	 pass2 table11 = new pass2(table3,mneumoics,table1,operands,address,args,reader,LTORGString);
	 //System.out.println();
	 //table11.display();
   }//end storeStuff
